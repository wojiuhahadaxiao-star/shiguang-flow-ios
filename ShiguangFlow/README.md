> **P3 补充功能**：待删除达到 99 张时弹出“查看并删除 / 继续浏览”，确认删除仍需原有确认流程；Live Photo 可在设置中开启 / 关闭，并选择静音 / 有声。默认关闭、静音，选择会保存。随机与当日均自动播放当前居中照片，全屏适屏时也可播放；放大后显示高清静态图。

> **1.1.1 P3**：上滑删除改为纯垂直位移并渐隐，不再缩小；移除删除后的全列表 reloadData，以增量删除 / 插入保留邻近照片；复用缩略图缓存，减少空白帧及低清回闪。待 GitHub 编译和实机验证。请完整更新源码及工作流后构建，产物和设置版本应为 1.1.1。
...
照片保持比例、不裁剪，以近似等面积排版并限制最大宽高。全屏“原图大小”按原比例适配屏幕理解，并非默认逐像素 1:1。放大清晰图按原比例请求，最长边最高 6000 像素，避免全尺寸超大照片带来过高内存占用。Live Photo 支持设置开关及静音 / 有声；普通视频不进入照片组。
